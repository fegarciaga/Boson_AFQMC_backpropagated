function [E_ave,E_err, Obs_bp_ave, Obs_bp_err, savedFileName]=PPMC_Bos_w_prof(Lx,Ly,Lz,N_par,kx,ky,kz,U,Uab,jj,tx,ty,tz,deltau,density_prof,N_wlk,N_blksteps,N_eqblk,N_blk,itv_pc,itv_nrm,itv_Em,t_bp,t_pop,suffix)
tic; % start the  timer
bosonic_initialization_w_prof; % initialize internal constants, form the trial wave function and assemble the initial population of walkers
format long;
...
for i_blk=1:N_eqblk
    for j_step=1:N_blksteps
        [Phi, Phi_old, B, B_W, parents, w, O, Obs, E, W, W_l] = bos_stepwlk(Phi, Phi_old, B, B_W, parents, N_wlk, N_sites, N, w, O, Obs, n_up, n_dn, E, W, W_l, K_old, Proj_k_half, flag_mea, flag_begin_bp, flag_bp, flag_bbp, Phi_T, N_par, t_bp, t_pop, U, Uab, fac_norm, deltau, ind_parents, ind_end, list);
...
for i_blk=1:N_blk
    for j_step=1:N_blksteps
        ...
        if ind_measure>0
            [Phi, Phi_old, B, B_W, parents, w, O, Obs_blk(ind_measure,:), E_blk(i_blk), W_blk(i_blk), W_bp(ind_measure)] = bos_stepwlk(Phi, Phi_old, B, B_W, parents, N_wlk, N_sites, N, w, O, Obs_blk(ind_measure,:), n_up, n_dn, E_blk(i_blk), W_blk(i_blk), W_bp(ind_measure), K_old, Proj_k_half, flag_mea, flag_begin_bp, flag_bp, flag_bbp, Phi_T, N_par, t_bp, t_pop, U, Uab, fac_norm, deltau, ind_parents+1, ind_end+1, list);
        else
            [Phi, Phi_old, B, B_W, parents, w, O, Obs, E_blk(i_blk), W_blk(i_blk), W] = bos_stepwlk(Phi, Phi_old, B, B_W, parents, N_wlk, N_sites, N, w, O, Obs, n_up, n_dn, E_blk(i_blk), W_blk(i_blk), W, K_old, Proj_k_half, flag_mea, flag_begin_bp, flag_bp, flag_bbp, Phi_T, N_par, t_bp, t_pop, U, Uab, fac_norm, deltau, ind_parents+1, ind_end+1, list);
        end
        if mod(j_step,itv_pc)==0
            [Phi, w, O, B, B_W, parents]=pop_cntrl_bos(Phi, w, O, N_wlk, N_sites, B, B_W, parents, N, t_bp, flag_bbp); % population control
        end
