...
% For selfconsistent study the previous profile is used:
Phi_T=zeros(2*N_sites,1);
Phi_T(1:N_sites)=-sqrt(density_prof(1:N_sites)/N_par);
Phi_T(1+N_sites:2*N_sites)=sqrt(density_prof(1+N_sites:2*N_sites)/N_par);
Phi_T=Phi_T/sqrt(Phi_T'*Phi_T);
E_T=measure_b(K, Phi_T, Phi_T, 1, N_sites, N_par, U, Uab);
...
fac_norm=real(E_T)*deltau+(0.5*U*((n_up'*n_up)+(n_dn'*n_dn))+0.5*Uab*real(n_up'*n_dn+n_dn'*n_up))*deltau;
